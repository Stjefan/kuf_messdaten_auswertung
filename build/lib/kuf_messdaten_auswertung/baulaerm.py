...
from messdaten.azure import MessdatenServiceV3, RandomMessdatenService
...
def erstelle_ergebnisse(lr_result_dict, beurteilungsrelevante_taktmaximalpegel_nach_laermursache, rejection_df: pd.DataFrame, number_fully_available_seconds,
    ...
    for idx, r in beurteilungsrelevante_taktmaximalpegel_nach_laermursache.iterrows():
        for mp, e in [(mp, f"{mp.Id}_{e}_TAKT") for mp in mps for e in mp.Ereignisse]:
            pass
...
def erstelle_baulaerm_auswertung(zeitpunkt_im_zielreitraum, project_id):
    ...
    resu = m.get_resu_all_mps(mps, from_date, to_date)

    terz = m.get_terz_all_mps(mps, from_date, to_date)
    dirs = m.get_dir_all_mps(mps, from_date, to_date)
    ...
    if True:
        ...
        richtungsfilter_ergebnisse, filterergebnisse_max_pegel = max_pegel_filter_04_23(
            mps, messwerte_nach_filtern, [range(mp.einstellungen_richtunsdaten.akzeptanz_untere_schranke, mp.einstellungen_richtunsdaten.akzeptanz_obere_schranke) for mp in mps])
        ...
        for io in ios:
            beurteilungspegel_an_io = erstelle_baulaerm_auswertung_an_io(zeitpunkt_im_zielreitraum, io, beurteilungsrelevante_taktmaximalpegel_nach_laermursache, [
                                                                         ausbreitungsfaktoren[(io.Id, mp.Id)] for mp in mps for k in mp.Ereignisse])
...
laermkategorisierung_an_messpunkten = [
    f"{k}_mp{mp.Id}" for mp in mps for k in mp.Ereignisse]
laermkategorisierung_an_immissionsorten = [
    f"{k}_mp{mp.Id}" for mp in mps for k in mp.Ereignisse] + ["Gesamt"]
...
laermkategorisierung_an_immissionsorten_extended = [
    {"name": f"{k}_mp{mp.Id}", "id": 1} for mp in mps for k in mp.Ereignisse] + ["Gesamt"]
...
laermkategorisierung_an_immissionsorten_reloaded = [{"name": f"Stihl MP {mp.Id} - Unkategorisiert", "name_messpunkt_kategorisierung": f"{k}_mp{mp.Id}", "column_name_ergebnis": name_column_beurteilungspegelrelevant.format(
    ereignis_name=k, mp_id=mp.Id), "id": None} for mp in mps for k in mp.Ereignisse] + [{"name": "Gesamt", "name_messpunkt_kategorisierung": None, "column_name_ergebnis": "Gesamt", "id": None}]
...
for i in laermkategorisierung_an_immissionsorten_datenbank:
    for ii in laermkategorisierung_an_immissionsorten_reloaded:
        if i["name"] == ii["name"]:
            ii["id"] = i["id"]


print(laermkategorisierung_an_immissionsorten_reloaded)
...
from_date = datetime(2023, 4, 16, 10, 0, 0)
to_date = from_date + timedelta(hours=3)

erstelle_baulaerm_auswertung(from_date, "8d7e0d22-620c-45b4-ac38-25b63ddf79e0")
